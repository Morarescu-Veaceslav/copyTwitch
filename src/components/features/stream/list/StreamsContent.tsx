...
import { useFindAllStreamsQuery, type FindAllStreamsQuery } from "@/graphql/generated/output";
...
export function StreamsContent({ streams }: StreamsContentProps) {
    ...
    const translation = useTranslations('streams')

    const searchParams = useSearchParams()
    const searchTerm = searchParams.get('searchTerm')

    const [streamList, setStreamList] = useState<
        FindAllStreamsQuery['findAllStreams']>(streams ?? [])
    ...
    return <>
        ...
        <Heading title={searchTerm ? `${translation('searchHeading')} "${searchTerm}"` : translation('heading')} />
        <InfinitiScroll
            dataLength={streamList.length}
            next={fetchMoreStreams}
            hasMore={hasMore}
            loader={<div className='mt-6 grid grid-cols-1 gap-8 md:grid-cols-2
