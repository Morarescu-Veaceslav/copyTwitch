...
export function ChangeAvatarForm() {
    ...
    const translation = useTranslations('dashboard.settings.profile.avatar')
    ...
    function handleImageChange(event: ChangeEvent<HTMLInputElement>) {
        ...
        if (file) {
            ...
            console.log('Mutation variables:', { avatar: file })
                            ...
                            <div className='w-full items-center space-x-6 lg:flex'>
                                ...
                                <div className='space-y-3'>
                                    <div className='flex items-center gap-x-3'>
                                        ...
                                        <Button variant='secondary' onClick={() => inputRef.current.click()} disabled={isLoadingUpdate || isLoadingRemove}>
                                            {translation('updateButton')}
                                        </Button>
                                        {/* {user?.avatar && (<Button variant='ghost' size='lgIcon' disabled={isLoadingUpdate || isLoadingRemove} onClick={() => remove()}>
                                            <Trash2 className='size-5' />
                                        </Button>)} */}
                                        {user?.avatar && (
                                            <ConfirmModal heading={translation('confirmModal.heading')} message={translation('confirmModal.message')} onConfirm={() => remove()}>
                                    ...
                                    <p className='text-sm text-muted-foreground'>{translation('info')}</p>
                                </div>
                            </div>
                        </div>
                    )}
