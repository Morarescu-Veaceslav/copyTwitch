...
export function ChannelItem({ channel }: ChannelItemProps) {
    ...
    return isCollapsed ? (
        ...
        <Button className={cn('mt-2 h-11 w-full justify-start hover:bg-accent/50', isActive && 'bg-accent/50')} variant='ghost' asChild>
            <Link href={`/${channel.username}`} className=' flex w-full items-center'>
                ...
                {channel.stream.isLive && <div className='absolute right-5'><LiveBadge /></div>}
            </Link>
        </Button>
    )
}
