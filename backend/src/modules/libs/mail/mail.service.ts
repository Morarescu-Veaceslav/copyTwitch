...
import { AccountDeletionTemplate } from './templates/account-deletion.tamplate';
...
export class MailService {
    ...
    public async sendEnableTwoFactor(email: string) {
        ...
        return this.sendMail(email, 'Enhance your account security with Two-Factor Authentication', html)
    }

    public async sendVerifyChannel(email: string) {
        const html = await render(VerifyChannelTemplate())

        return this.sendMail(email, 'Your channel has been verified on TwitchTest', html)
    }
