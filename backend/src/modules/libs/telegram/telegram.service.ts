...
export class TelegramService extends Telegraf {
    ...
    public async onMe(@Ctx() ctx: Context) {
        ...
        if (!chatId) {
            await ctx.reply('Eroare: nu am putut obține chatId-ul tău.');
            return;
        }
        ...
        if (!user) {
            await ctx.reply('Nu am găsit niciun cont asociat acestui chat Telegram.');
            return;
        }

        // Nu mai trimitem MESSAGES.authSuccess aici
    ...
    public async onFollows(@Ctx() ctx: Context) {
        ...
        if (user && follows.length) {
            ...
            const message = `<b>Canalele la care sunteti abonat:</b> \n\n ${followsList}`

            await ctx.replyWithHTML(message)

        } else {
            await ctx.replyWithHTML('📭 <b>Nu aveti abonari.</b>')
