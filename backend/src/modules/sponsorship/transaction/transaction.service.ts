    ...
    public async makePayment(user: User, planId: string) {
        ...
        if (!plan) {
            throw new GraphQLError('Plan not found.', {
                extensions: {
                    code: 'MAKE_PAYMENT_ERROR'
                }
            });
        }

        if (user.id === plan.channel?.id) {
            throw new GraphQLError('You cannot sponsor your own channel.', {
                extensions: {
                    code: 'MAKE_PAYMENT_ERROR'
                }
            });
        ...
        const existingSubscription = await this.prismaService.sponsorshipSubstription.findFirst({
            where: {
                userId: user.id,
        ...
        const costomer = await this.stripeService.customers.create({
            name: user.username,
            email: user.email
        })
